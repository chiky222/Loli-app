// ── Storage seguro (funciona aunque localStorage falle) ──
var store = (function(){
var mem = {};
function ok(){ try{ localStorage.setItem(’_t’,‘1’); localStorage.removeItem(’_t’); return true; }catch(e){ return false; } }
var ls = ok();
return {
get: function(k){ if(ls){ var v=localStorage.getItem(k); return v?JSON.parse(v):null; } return mem[k]||null; },
set: function(k,v){ if(ls){ try{ localStorage.setItem(k,JSON.stringify(v)); }catch(e){} } mem[k]=v; }
};
})();
...
// ── Carga ──
function loadAll(){
rifaData      = store.get(‘rifa-data’)   || {};
sorteoHistory = store.get(‘rifa-sorteo’) || [];
}
function saveData()    { store.set(‘rifa-data’,   rifaData); }
function saveHistory() { store.set(‘rifa-sorteo’, sorteoHistory); }
...
saveData(); buildGrid(); renderList(); closeModal();
showToast(’N ’ + String(currentNum).padStart(2,‘0’) + ’ vendido a ’ + name);
}
function editBuyer(){
...
list.innerHTML = ‘<div class="empty-state"><div class="empty-icon">’ + (currentFilter===‘unpaid’?‘🎉’:‘🎟️’) + ‘</div><p>’ + (sv ? ‘Sin resultados.’ : msg) + ‘</p></div>’;
return;
}
list.innerHTML = entries.map(function(e){
var n = e[0], b = e[1];
return ‘<div class="sold-item">’ +
‘<div class="sold-num ' + (b.paid ? 'is-paid' : 'is-unpaid') + '">’ + n + ‘</div>’ +
...
‘</div>’ +
‘<button class="btn-mark-paid" onclick="quickToggle(' + k + ')">Cobrado</button>’ +
‘</div>’;
}).join(’’);
}
...
lastWinner = {num: num};
sorteoHistory.unshift({num: num, buyer: buyer, date: new Date().toLocaleString(‘es-AR’)});
saveHistory(); renderHistory(); buildGrid();
launchBurst();
sorteoRunning = false;
...
{transform:’translate(calc(-50% + ’+tx+’px),calc(-50% + ’+ty+‘px)) scale(1) rotate(’+(ang*3)+‘deg)’,opacity:0}
